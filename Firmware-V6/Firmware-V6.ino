...
Student #: 101075941
Current Version Date: 2021-01-14
...
#define STEP_LIMIT 150 //steps/second limit
...
float azPos = 0;  //Azimuth position according to sensors(deg)
float elPos = 0;  //Elevation position according to sensors(deg)
...
void setup() {
  ...
  Serial.println("Active"); //Show that the system is active
  Serial.flush();

  //setting max step rate of motors:
  //This is not the speed of the motors, it's the max possible
  AzStepper.setMaxSpeed(150); //This used to be set to 500 and for some reason it
  ElStepper.setMaxSpeed(150);  //made the system unstable
...
void loop() {
  ...
  if (chCount != 0 && Serial.available() < 1){
    if(inChar[0] == 'A' && inChar[2] != ' '){
      ...
      //This is the flush block sent by Hamlib, occurs "randomly"
      //response is to do nothing
      Serial.println("Stop Command"); //not required
      ...
      //combines the 4 above values into the outChar char array:
      sprintf(outChar, "AZ%d.%d EL%d.%d", azInt, azFlt, elInt, elFlt);
      Serial.write(outChar); //write the outChar variable to serial port
      Serial.println("");
      Serial.flush(); //holds program while Serial.write sends data
    }else if(inChar[0] == 'C'){
      //This is a user-called function that will be used to calibrate the system
      calibrate();
    }else if(inChar[0] == 'T'){
      //This is a user-called function that prints the homing switch states
      while(HIGH){
        delay(1000);
      ...
      //Occurs when an unknown command is receieved
      Serial.println("err");
    }
...
void sensorPos(){
  ...
  azPos = (AzStepper.currentPosition() * AZ_DEG_PER_STEP) + n; //FIXME
  ...
  elPos = ElStepper.currentPosition() * EL_DEG_PER_STEP; //map function was weird
...
void setTgt(){
  ...
  //Rip the azimuth target value from the inChar array
  ...
  //add bias
  if((azTgt + azBias) > 360){
    azTgt = azTgt + azBias - 360;
  }else if((azTgt - azBias) < 0){
    azTgt = azTgt + azBias + 360;
  }else{
    azTgt = azTgt + azBias;
  }

  //Rip the elevation target value from the inChar array
  indFrom = 'L';
  valLen = getLenBetween(inChar, indFrom, indTo);
  carry = getIndOf(inChar, indFrom);
  elTgt = excludingPlaces(inChar, carry+1, carry+valLen);

  //if((elTgt - elBias) < 0){elTgt = 0;}
  if((elTgt + elBias) > 90){elTgt = 90;}
  else{elTgt = elTgt + elBias;}
